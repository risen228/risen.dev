...
Redux, unlike Effector and many other state-managers, by itself doesn't provide a convenient way to get and combine data from the store. Selectors were created to address this problem. And they act as sort of getters, extracting specific parts of the state.
 ...
 * we'll have to update all 3 lines of code.
 * 
...
After seeing this library for the first time, a lot of people start using `createSelector` literally everywhere. Of course, this is the wrong approach. Memoized selectors are good only in some situations.
...
It means, for example, when user opens/closes some global modal, which state lies in redux store, we will re-calculate all the values coming from not memoized selectors, even if the shopping cart is not relying on this modal at all.
...
This selector isn't doing any heavy computations, but it returns an object.

Without memoization, on each call the new object will be returned, even when input data remains the same. `mapStateToProps` and `useSelector` will count it as data change and component will re-render.

A memoized selector, on the other hand, will return the same reference all the time, until the input data has changed. So, there won't be any extra re-renders.

This can be applied to any values which are object technically (arrays, instances of `Date`, `Map`, `Set`, and so on). We will call it just an "object" for simplicity.
...
In situations like this, I prefer using `createSelector` version, even if it's not so fast and takes some memory. It's just not a big problem in context of entire application.
...
Don't worry even if the return type is object - you will just get the reference to an already existing object, which is located in the store. It won't lead to any problems.

Memoized selectors are not good in cases like this. First, they are about 30 times slower. And second, they need some memory for storing previous computations data. Those problems are not so visible, but they can become like so if you use memoized selectors everywhere.

## useSelector in details {#use-selector-in-details}

It may seem that `useSelector` allows us to not use memoized selectors, but it is not.

The first reason: the function passed into it is executed for every change in the store. It means that every heavy computation will re-calculate, no matter what slice of the state was changed. `useSelector` has no way to compare the input data, since it just takes the function with a single parameter `state`.

The second reason: it would be a bad idea to throw out a composition (that was described [there](#data-mapping-and-composition)). We can't do the same thing without `reselect`, or else we'll have to write our own helper.
